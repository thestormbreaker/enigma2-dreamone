...
DEVICEDB = {'dm8000': {'/devices/pci0000:01/0000:01:00.0/host1/target1:0:0/1:0:0:0': _('SATA'),
 ...
 'dm820': {'/devices/platform/ehci-brcm.0/': _('Back, lower USB'),
           ...
           '/devices/platform/strict-ahci.0/ata1/': _('SATA'),
           '/devices/platform/strict-ahci.0/ata2/': _('SATA')},
 'dm520': {'/devices/platform/ehci-brcm.0/usb1/1-2/': _('Back, outer USB'),
 ...
 'dm900': {'/devices/platform/brcmstb-ahci.0/ata1/': _('SATA'),
 ...
 'dm920': {'/devices/platform/brcmstb-ahci.0/ata1/': _('SATA'),
           '/devices/rdb.4/f03e0000.sdhci/mmc_host/mmc0/': _('eMMC'),
           '/devices/rdb.4/f03e0200.sdhci/mmc_host/mmc1/': _('SD'),
           '/devices/rdb.4/f0470600.ohci_v2/usb6/6-0:1.0/port1/': _('Front USB'),
           '/devices/rdb.4/f0470300.ehci_v2/usb3/3-0:1.0/port1/': _('Front USB'),
...
class HarddiskManager():
    ...
    def getBlockDevInfo(self, blockdev):
        ...
        if blockdev[:7] in BLACKLIST:
            blacklisted = True
        if blockdev.startswith('mmcblk') and (re.search('mmcblk\\dboot', blockdev) or re.search('mmcblk\\drpmb', blockdev)):
            blacklisted = True
        is_cdrom = False
        partitions = []
        try:
            ...
            if not is_cdrom and os.path.exists(devpath):
                for partition in os.listdir(devpath):
                    if partition[0:len(blockdev)] != blockdev:
                        continue
                    if dev == 179 and not re.search('mmcblk\\dp\\d+', partition):
                        continue
    ...
    def addHotplugPartition(self, device, physdev = None):
        ...
        if not blacklisted and medium_found:
            ...
            l = len(device)
            if l and (not device[l - 1].isdigit() or device.startswith('mmcblk') and not re.search('mmcblk\\dp\\d+', device)):
    ...
    def splitDeviceName(self, devname):
        if re.search('^mmcblk\\d(?:p\\d+$|$)', devname):
            m = re.search('(?P<dev>mmcblk\\d)p(?P<part>\\d+)$', devname)
            if m:
